...
namespace Photon.Pun.Demo.Asteroids
{
    public class GameSystem : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            {
                Debug.LogFormat("We are Instantiating LocalPlayer from Main Railing Scene");
                Debug.LogFormat("<Color=Red><a>Important!</a></Color>, value of ActorNumber is: " + PhotonNetwork.LocalPlayer.ActorNumber);
                // we're in a room. spawn a character for the local player. it gets synced by using PhotonNetwork.Instantiate
                if(PhotonNetwork.LocalPlayer.ActorNumber == 1)
                {
                    Debug.Log("Instantiating: " + PhotonNetwork.LocalPlayer.ActorNumber);
                    PhotonNetwork.Instantiate(this.playerPrefab.name, new Vector3(21.25f, 3.5f, -22.22f), Quaternion.identity, 0);
                }
                else if (PhotonNetwork.LocalPlayer.ActorNumber == 2)
                {
                    Debug.Log("Instantiating: " + PhotonNetwork.LocalPlayer.ActorNumber);
                    PhotonNetwork.Instantiate(this.playerPrefab.name, new Vector3(23.10f, 3.5f, -22.08f), Quaternion.identity, 0);
                }
                else if (PhotonNetwork.LocalPlayer.ActorNumber == 3)
                {
                    Debug.Log("Instantiating: " + PhotonNetwork.LocalPlayer.ActorNumber);
                    PhotonNetwork.Instantiate(this.playerPrefab.name, new Vector3(23.26f, 3.5f, -24.63f), Quaternion.identity, 0);
                }
                else if (PhotonNetwork.LocalPlayer.ActorNumber == 4)
                {
                    Debug.Log("Instantiating: " + PhotonNetwork.LocalPlayer.ActorNumber);
                    PhotonNetwork.Instantiate(this.playerPrefab.name, new Vector3(21.52f, 3.5f, -24.77f), Quaternion.identity, 0);
                }
            }
